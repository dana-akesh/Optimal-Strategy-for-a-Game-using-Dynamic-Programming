...
public class StartupUI implements Runnable {
    ...
    private final MainUI mainUI = new MainUI(algorithm);
    private Stage stage;
    ...
    public StartupUI() {
        ...
        insertLabel.setFont(Font.font("Georgia", FontWeight.BOLD, 18));
        insertLabel.setStyle("-fx-text-fill: #54494b");

        Label noOfCoinsLabel = new Label("The number of coins is: " + 0);
        noOfCoinsLabel.setFont(Font.font("Georgia", FontWeight.BOLD, 14));
        ...
        insertBtn.setFont(Font.font("Georgia", FontWeight.BOLD, 16));
        ...
        TranslateTransition coinImgTR = new TranslateTransition();
        coinImgTR.setNode(coinImg);
        coinImgTR.setByY(-10);
        coinImgTR.setAutoReverse(true);
        coinImgTR.setDuration(Duration.seconds(1.5));
        coinImgTR.setCycleCount(Animation.INDEFINITE);
        coinImgTR.play();
        ...
        TranslateTransition startBtnTR = new TranslateTransition();
        startBtnTR.setNode(startBtn);
        startBtnTR.setByX(40);
        startBtnTR.setByY(30);
        startBtnTR.play();
    ...
    public void run() {
        for (int i = 0; i < algorithm.coinsArrayList.size(); i++) {
            mainUI.getCoinsHB().getChildren().add(algorithm.coinsArrayList.get(i));
        }
    }
